...
BEGIN
    RETURN QUERY
        WITH scrpts AS (
            SELECT csm.commentid, csm.linesfrom, csm.linesto, array(select string_agg(cde.data, '')
    ...
    SELECT c.id, c.data, c.root, u.firstname, u.lastname, u.id, sc.linesfrom, sc.linesto, sc.lns
    FROM course.comment c
    INNER JOIN scrpts sc ON c.id = sc.commentid
